...
while(setpoint == 0):
        ...
        try:
                setpoint = (int(escrito))  #sempre um valor entre 90 e 225
        except:
                if escrito == 'r':
                        ser.write([7,255,3])
                elif escrito == 'p':
...
#def vetor a ser inserido no treinamento
        sets = numpy.array([[setpoint/225., erro/140.]])

#def resposta e limites de resposta
        pwm = int((anfis.predict(anf, sets))*255)
        print(pwm)
        if  pwm > 40:
                modo = 1
                plot_final = False
...
#for i in range(len(tempo)):
 #       with open('planta_neuro5.txt','a') as arquivo:
